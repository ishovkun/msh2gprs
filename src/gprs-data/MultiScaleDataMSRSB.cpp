...
void MultiScaleDataMSRSB::build_metis_partitioning()
{
  ...
  for (auto it = grid.begin_faces(); it != grid.end_faces(); ++it)
  {
    ...
    if (neighbors.size() == 2)  // not a boundary face
      cell_connections.insert( neighbors[0], neighbors[1] );
  }
...
void MultiScaleDataMSRSB::build_block_connections()
{
  ...
  for (auto face = grid.begin_faces(); face != grid.end_faces(); ++face)
  {
    ...
    if (neighbors.size() == 2)  // not a boundary face
    {
      ...
      if (i1 != i2)
        if (layer.block_internal_connections.find(i1, i2) == layer.block_internal_connections.end())
...
find_block_face_centroids(const std::unordered_map<size_t,size_t> & map_boundary_face_ghost_block)
{
  ...
  for (auto face = grid.begin_faces(); face != grid.end_faces(); ++face)
  {
    ...
    size_t conn_ind;
    if (layer.block_faces.find(block1, block2) != layer.block_faces.end())
      ...
      &map_block_edge_vertices)
{
  ...
  for (const auto & block_edge : map_block_edge_vertices)
  {
    ...
    for (std::size_t i = 0; i < 3; ++i)
    {
      ...
      BlockFace * p_face;
      if (layer.block_faces.find(i1, i2) != layer.block_faces.end())
                     ...
                     const hash_algorithms::ConnectionMap<BlockFace> & block_faces)
{
  ...
  {
    if (block_connections.contains(nn1, n1) &&
        block_connections.contains(nn1, n2))
    {
      // std::cout << "better = " << nn1 <<" ";
      return true;
    }
    ...
    if (!is_ghost_block(neighbor1))
    {
      ...
      for (const size_t & neighbor2 : layer.block_faces.get_neighbors(neighbor1))
      {
        ...
        for (const size_t & neighbor3 : face_n1_n2.edging_blocks)
        {
          ...
          // and again
          if (layer.block_faces.find(neighbor1, neighbor3) == layer.block_faces.end())
          {
